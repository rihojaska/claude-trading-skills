...
def _restore_fmp_get():
    # _client() rebinds mod.fmp_get to a per-test fake; restore afterwards.
    orig = mod.fmp_get
    yield
    mod.fmp_get = orig
...
def _client(router):
    """Build a client whose transport is routed through `router`.

    FMPClient._request goes through fmp_compat.fmp_get (not raw
    requests.Session), so the fake transport is injected there. Calls are
    still recorded on `session.get` — same shape the assertions were written
    against (args[0] = url, kwargs["params"]) — and the router's _resp mocks
    are translated to fmp_get semantics: 200 -> parsed JSON, else None.
    """
...
class TestGetRouting:
    ...
    def test_dividends_normalized_to_historical(self):
        # >=16 records so get_dividend_history trusts the FMP series and does
        # not fall back to yfinance (free-tier truncation guard).
        flat = [
            {"symbol": "AAPL", "date": f"{2026 - i // 4}-{(3 * (i % 4)) + 2:02d}-11", "dividend": 0.27}
            for i in range(16)
        ]
